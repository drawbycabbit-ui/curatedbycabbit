...
// Data produk untuk kebutuhan filter admin
let adminProducts = [];

// Daftar kategori, toko, dan tags unik untuk dropdown filter admin
...
productForm.addEventListener('submit', async (e) => {
    ...
    if (!id) {
        ...
        // clickCount dipakai untuk sorting "Most Clickable" di halaman utama
...
/**
AMBIL DATA PRODUK UNTUK ADMIN

Sekarang fungsi ini juga menyiapkan:
- array adminProducts untuk filter
- dropdown kategori, toko, dan tags
*/
async function loadAdminProducts() {
    ...
    try {
        ...
        querySnapshot.forEach((docSnap) => {
            ...
            // Simpan objek asli ke cache untuk edit/delete
            productsCache[p.id] = p;

            // Simpan ke array untuk filter admin
            adminProducts.push(p);
        });

        // Urutkan produk admin berdasarkan terbaru
        adminProducts.sort((a, b) => {
            return getAdminTime(b) - getAdminTime(a);
        });

        // Ekstrak kategori, toko, dan tags unik
...
// --- 3. FILTER & PENCARIAN ADMIN ---
/**
FILTER & PENCARIAN ADMIN

Digunakan untuk mencari dan memfilter daftar produk di halaman admin:
- search keyword
- kategori / brand
- toko
- tags
...
// Helper waktu untuk sorting produk admin
function getAdminTime(product) {
    return product.createdAt?.seconds || product.updatedAt?.seconds || 0;
}

// Ekstrak kategori, toko, dan tags unik dari data produk admin
...
function renderAdminFilterOptions() {
    ...
    adminUniqueCategories.forEach(cat => {
        adminFilterCategory.innerHTML += `<option value="${escapeHTML(cat)}">${escapeHTML(cat)}</option>`;
    });
    ...
    adminUniqueTags.forEach(tag => {
        adminFilterTag.innerHTML += `<option value="${escapeHTML(tag)}">#${escapeHTML(tag)}</option>`;
    });
...
function applyAdminFilters() {
    // Jika elemen filter belum ada di HTML, tampilkan semua produk
    ...
    const filtered = adminProducts.filter(product => {
        ...
        // Cari berdasarkan nama produk, kategori, toko, dan tags agar lebih fleksibel
...
// Render tabel produk admin berdasarkan hasil filter
    ...
    products.forEach(p => {
        ...
        row.innerHTML = `
            ...
            <td class="p-4">
                ...
                <!-- Opsional: menampilkan jumlah klik affiliate -->
...
// --- 4. EVENT LISTENERS FILTER ADMIN ---
if (adminSearchInput) {
