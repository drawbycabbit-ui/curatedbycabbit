...
products.forEach(product => {
    // Buat elemen kartu produk (Versi Tanpa Harga)
    ...
    card.innerHTML = `
        ...
        <div class="p-4 flex flex-col flex-grow">
            <p class="text-xs font-bold text-primary uppercase tracking-wider mb-1">${product.brand || 'Produk Pilihan'}</p>
            <h3 class="font-brand font-semibold text-dark mb-4 line-clamp-2 flex-grow">${product.name}</h3>
            
            <!-- Tombol diganti teksnya karena tidak ada harga -->
            <a href="${product.affiliateLink}" target="_blank" rel="noopener noreferrer" 
               class="w-full bg-primary hover:bg-opacity-90 text-white font-bold py-2.5 px-4 rounded-lg text-center transition flex items-center justify-center gap-2">
                <span>Cek Detail di Marketplace</span>
